...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Quickchain
...
type wallet struct {
	...
	deriveChain    quickchain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Quickchain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain quickchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
//
// Note, if the version of the Quickchain application running on the Ledger wallet is
