...
// UUID represents the 16 byte address.
...
func BigToUUID(b *big.Int) UUID { return BytesToUUID(b.Bytes()) }
func HexToUUID(s string) UUID   { return BytesToUUID(FromHex(s)) }
...
func IsHexUUID(s string) bool {
	...
	return len(s) == 2*UUIDLength && isHex(s)
}

// Get the string representation of the underlying UUID
func (a UUID) Str() string   { return string(a[:]) }
...
// Format implements fmt.Formatter, forcing the byte slice to be formatted as is,
// without going through the stringer interface used for logging.
func (a UUID) Format(s fmt.State, c rune) {
	fmt.Fprintf(s, "%"+string(c), a[:])
}

// Sets the UUID to the value of b. If b is larger than len(a) it will panic
...
// Set string `s` to a. If s is larger than len(a) it will panic
func (a *UUID) SetString(s string) { a.SetBytes([]byte(s)) }

// Sets a to other
func (a *UUID) Set(other UUID) {
	for i, v := range other {
		a[i] = v
	}
}
