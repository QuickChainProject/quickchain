...
// the genesis block.
func newSnapshot(epoch uint64, number uint64, hash common.Hash, valSet bft.ValidatorSet) *Snapshot {
	snap := &Snapshot{
		Epoch:  epoch,
		Number: number,
		Hash:   hash,
...
func loadSnapshot(epoch uint64, db qctdb.Database, hash common.Hash) (*Snapshot, error) {
