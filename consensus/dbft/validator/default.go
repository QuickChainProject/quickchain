...
func newDefaultSet(addrs []common.Address, policy bft.ProposerPolicy) *defaultSet {
	valSet := &defaultSet{}
...
func roundRobinProposer(valSet bft.ValidatorSet, proposer common.Address, round uint64) bft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func stickyProposer(valSet bft.ValidatorSet, proposer common.Address, round uint64) bft.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func (valSet *defaultSet) Policy() bft.ProposerPolicy { return valSet.policy }
