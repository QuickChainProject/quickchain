...
// ecrecover extracts the Quickchain account address from a signed header.
/*
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the Quickchain address
...
type DPOS struct {
	...
	signer common.Address // Quickchain address of the signing key
...
func (c *DPOS) DB() qctdb.Database { return c.db  }

// Author implements consensus.Engine, returning the Quickchain address recovered
...
func (c *DPOS) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if _, ok := epoch.Signers[signer]; !ok {
		log.Error("verifySeal", "signer", signer, "Signers", epoch.Signers)
	...
	for seen, recent := range epoch.Recents {
		if recent == signer {
			...
			if limit := uint64(len(epoch.Signers)/2 + 1); seen > number-limit {
				log.Error("Recents", "limit", limit, "seen", seen, "number", number)
				return errUnauthorized
			}
		}
	}
...
func (c *DPOS) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	//epoch, err := c.epochContext(chain, number-1, header.ParentHash, parent)
	_, err := c.epochContext(chain, number-1, header.ParentHash, parent)
	if err != nil {
		return nil, err
	}
	/*
	if _, authorized := epoch.Signers[signer]; !authorized {
		log.Error("Seal", "signer", signer, "Signers", epoch.Signers)
