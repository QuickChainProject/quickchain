// Copyright 2017 The quickchain Authors
// This file is part of the quickchain library.
//
// The quickchain library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// The quickchain library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with the quickchain library. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/quickchainproject/quickchain/consensus/misc"
...
type PoA struct {
	...
	db     qctdb.Database    // Database to store and retrieve snapshot checkpoints
...
func New(config *params.POAConfig, db qctdb.Database) *PoA {
...
func (c *PoA) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *PoA) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
...
func (c *PoA) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *PoA) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (c *PoA) verifyCascadingFields(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
...
func (c *PoA) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *PoA) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *PoA) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
func (c *PoA) Prepare(chain consensus.ChainReader, header *types.Header) error {
	//log.Info("Prepare")
...
// with.
func (c *PoA) Authorize(signer common.Address, signFn SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *PoA) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	header := block.Header()
...
// current signer.
func (c *PoA) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
...
func (c *PoA) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "poa",
		Version:   "1.0",
		Service:   &API{chain: chain, poa: c},
		Public:    false,
	}}
}
