...
# Install quickchain binary
Section "Quickchain" QUICKCHAIN_IDX
  SetOutPath $INSTDIR
  file {{.Quickchain}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\quickchain.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\quickchain.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Quickchain incoming peers (TCP:36663)"
  SimpleFC::AdvRemoveRule "Quickchain outgoing peers (TCP:36663)"
  SimpleFC::AdvRemoveRule "Quickchain UDP discovery (UDP:36663)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Quickchain incoming peers (TCP:36663)" ""  6 1 1 2147483647 1 "$INSTDIR\quickchain.exe" "" "" "Ethereum" 36663 "" "" ""
  SimpleFC::AdvAddRule "Quickchain outgoing peers (TCP:36663)" ""  6 2 1 2147483647 1 "$INSTDIR\quickchain.exe" "" "" "Ethereum" "" 36663 "" ""
  SimpleFC::AdvAddRule "Quickchain UDP discovery (UDP:36663)" "" 17 2 1 2147483647 1 "$INSTDIR\quickchain.exe" "" "" "Ethereum" "" 36663 "" ""

  # Set default IPC endpoint (https://github.com/quickchain/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\qct.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\qct.ipc"
